...
function buildDockerImage () {
  ...
  // Could call buildCustomDockerImage() here if we wanted to pass in custom run commands later on
  // fse.readFileSync(dockerFilePath).toString()
  
  const dockerOptions = ['build', '-f', dockerFilePath, '-t', this.provider.naming.getDockerImageName(), '.']
...
function buildDockerImageFromDockerfile () {
  ...
  const dockerfilePath = `${__dirname}/../../../Dockerfile`
  const dockerFileContents = fse.readFileSync(dockerfilePath).toString()
